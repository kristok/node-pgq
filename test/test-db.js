...
describe('db.js', function() {
	...
	describe('query.first', function() {
		it('executes query and returns first row if it exists', function(done) {
			var expectedResult = {
					rows: [{hi: 'first'}, {hi: 'second'}]
				},
				expectedDone = sinon.spy(),
				client = {};
			...
			subject({}).query.first('', [], function(err, result) {
				assert.equal(result, expectedResult.rows[0]);
				assert.ok(expectedDone.calledOnce);
				done();
			});
		});

		it('executes query and returns null if no results', function(done) {
			var expectedResult = {},
				expectedDone = sinon.spy(),
				client = {};
