...
var PgQDatabaseApi = function(config) {
	...
	var loadBatchEventsAtOnce = function(params, callback) {
		var query = [
			...
			'       ev_type as type,',
			'       ev_data as data,',
			'       ev_extra1 as prev_data,',
			'       ev_extra2 as table,',
			'       ev_extra3 as primary_keys,',
			'       ev_extra4 as operation',
			'  FROM pgq.get_batch_events($1::int)'
		].join('\n');
	...
	self.addTableTrigger = function(tableName, queueName, callback) {
		...
		escapeTableTriggerParams(tableName, queueName, function(err, escaped) {
			...
			var query = [
				...
				'FOR EACH ROW',
				'EXECUTE PROCEDURE pgq.trigger_log_json(' + escaped.queue + ');'
			].join('\n');
	...
	self.sendQuery = function(query, callback) {
		self.db.query(query, [], callback);
	};
